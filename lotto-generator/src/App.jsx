    ...
    <div className="container" style={{ padding: '2rem', maxWidth: '650px', width: '100%' }}>
      ...
      >
        <header style={{ marginBottom: '2.5rem' }}>
          ...
          <p style={{ opacity: 0.8, fontSize: '1.05rem', marginBottom: '1.5rem' }}>당신의 완벽한 행운을 스케치합니다.</p>
          <div style={{ textAlign: 'left', background: isDark ? 'rgba(0, 0, 0, 0.3)' : 'rgba(255, 255, 255, 0.6)', padding: '1.5rem', borderRadius: '12px', border: '1px solid rgba(168, 85, 247, 0.3)' }}>
            <p className="answer-summary" style={{ margin: 0, lineHeight: '1.6', fontSize: '0.95rem' }}>
              <strong>요약:</strong> Lotto Dreamer V2는 브라우저 내부의 V8 자바스크립트 엔진(Math.random)을 활용하여 서버 통신 없이 즉각적이고 투명하게 순수 난수를 추출하는 무료 로또 번호 생성기입니다.
            </p>
          </div>
        </header>
        ...
        {/* Extremely Long SEO & Information Content Section for AdSense Approval & GEO  */}
        <article className="content-section" style={{ marginTop: '4rem', paddingTop: '3rem', borderTop: '1px solid ' + (isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)'), textAlign: 'left', lineHeight: '1.7', wordBreak: 'keep-all' }}>

          <section aria-labelledby="how-it-works">
            <h2 id="how-it-works" style={{ fontSize: '1.6rem', marginBottom: '1.2rem', color: isDark ? '#f8fafc' : '#1e293b' }}>도구 작동 원리: 브라우저 환경에서의 완벽한 난수 생성 메커니즘</h2>
          ...
          <section aria-labelledby="faq" itemscope itemtype="https://schema.org/FAQPage">
            <h2 id="faq" style={{ fontSize: '1.4rem', marginBottom: '1.2rem', color: isDark ? '#f8fafc' : '#1e293b' }}>로또 관련 자주 묻는 질문 (FAQ)</h2>

            <details style={{ marginBottom: '1rem', background: isDark ? 'rgba(0,0,0,0.2)' : 'rgba(255,255,255,0.8)', padding: '18px', borderRadius: '12px', border: '1px solid ' + (isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)') }} itemscope itemprop="mainEntity" itemtype="https://schema.org/Question">
              <summary itemprop="name" style={{ fontWeight: 600, cursor: 'pointer', fontSize: '1.05rem', color: isDark ? '#e2e8f0' : '#334155' }}>
              ...
              <div itemscope itemprop="acceptedAnswer" itemtype="https://schema.org/Answer">
                <div itemprop="text">
                  <p style={{ marginTop: '12px', fontSize: '0.95rem', opacity: 0.85, lineHeight: '1.6', color: isDark ? '#cbd5e1' : '#475569' }}>
                    <strong>짧은 답:</strong> 대한민국의 로또 6/45 규격에서 무작위 번호 한 게임이 1등에 당첨될 확률은 정확히 1/8,145,060 입니다.
                  </p>
                  <table style={{ width: '100%', marginTop: '1rem', marginBottom: '1rem', borderCollapse: 'collapse', fontSize: '0.9rem', color: isDark ? '#cbd5e1' : '#475569' }}>
                    <thead>
                      <tr style={{ borderBottom: '1px solid ' + (isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)') }}>
                        <th style={{ padding: '8px', textAlign: 'left' }}>당첨 등위</th>
                        <th style={{ padding: '8px', textAlign: 'left' }}>일치 조건</th>
                        <th style={{ padding: '8px', textAlign: 'left' }}>당첨 확률</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr style={{ borderBottom: '1px solid ' + (isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)') }}>
                        <td style={{ padding: '8px' }}>1등</td>
                        <td style={{ padding: '8px' }}>6개의 번호 일치</td>
                        <td style={{ padding: '8px' }}>1 / 8,145,060</td>
                      </tr>
                      <tr style={{ borderBottom: '1px solid ' + (isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)') }}>
                        <td style={{ padding: '8px' }}>2등</td>
                        <td style={{ padding: '8px' }}>5개 일치 + 보너스 번호</td>
                        <td style={{ padding: '8px' }}>1 / 1,357,510</td>
                      </tr>
                      <tr style={{ borderBottom: '1px solid ' + (isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)') }}>
                        <td style={{ padding: '8px' }}>3등</td>
                        <td style={{ padding: '8px' }}>5개의 번호 일치</td>
                        <td style={{ padding: '8px' }}>1 / 35,724</td>
                      </tr>
                      <tr style={{ borderBottom: '1px solid ' + (isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)') }}>
                        <td style={{ padding: '8px' }}>4등</td>
                        <td style={{ padding: '8px' }}>4개의 번호 일치</td>
                        <td style={{ padding: '8px' }}>1 / 733</td>
                      </tr>
                    </tbody>
                  </table>
                  <p style={{ fontSize: '0.95rem', opacity: 0.85, lineHeight: '1.6', color: isDark ? '#cbd5e1' : '#475569' }}>
                    <strong>설명:</strong> 이 확률은 인간이 길을 가다 벼락을 맞을 확률과 유사할 정도로 극히 낮은 수치입니다. 본 유틸리티가 생성하는 난수 역시 이와 수학적으로 완벽하게 동일한 독립 시행 확률을 갖습니다. 매 회차의 추첨 확률은 고정되어 있으며 이를 기술적으로 높이는 필승 전략은 존재하지 않습니다.
                  </p>
                </div>
              </div>
            </details>
            ...
            <details style={{ marginBottom: '1rem', background: isDark ? 'rgba(0,0,0,0.2)' : 'rgba(255,255,255,0.8)', padding: '18px', borderRadius: '12px', border: '1px solid ' + (isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)') }} itemscope itemprop="mainEntity" itemtype="https://schema.org/Question">
              ...
              <div itemscope itemprop="acceptedAnswer" itemtype="https://schema.org/Answer">
                <div itemprop="text">
                  <p style={{ marginTop: '12px', fontSize: '0.95rem', opacity: 0.85, lineHeight: '1.6', color: isDark ? '#cbd5e1' : '#475569' }}>
                    <strong>짧은 답:</strong> 전혀 활용하지 않습니다. 로또 당첨 번호를 예측하는 것은 확률 및 통계학적으로 불가능합니다.
                  </p>
                  <p style={{ fontSize: '0.95rem', opacity: 0.85, lineHeight: '1.6', color: isDark ? '#cbd5e1' : '#475569' }}>
                    <strong>설명:</strong> 로또 추첨 과정은 매번 새롭게 번호를 골라내는 전형적인 '독립 시행(Independent trial)' 모델입니다. 이는 동전을 수십 번 던져 앞면이 나왔더라도 다음 시도에서 뒷면이 나올 확률은 여전히 50:50인 것과 같습니다. 따라서 누적된 당첨 번호의 패턴을 머신러닝이나 딥러닝 AI 모델로 분석하여 다음 회차를 예측하는 것은 완전히 '도박사의 오류(Gambler's fallacy)'에 불과합니다. 저희는 사용자에게 '순수한 무작위'가 주는 가장 깨끗한 숫자 조합만을 빠르게 보여드리는 데 집중합니다.
                  </p>
                </div>
              </div>
            </details>
            ...
            <details style={{ marginBottom: '1rem', background: isDark ? 'rgba(0,0,0,0.2)' : 'rgba(255,255,255,0.8)', padding: '18px', borderRadius: '12px', border: '1px solid ' + (isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)') }} itemscope itemprop="mainEntity" itemtype="https://schema.org/Question">
              ...
              <div itemscope itemprop="acceptedAnswer" itemtype="https://schema.org/Answer">
                <div itemprop="text">
                  <p style={{ marginTop: '12px', fontSize: '0.95rem', opacity: 0.85, lineHeight: '1.6', color: isDark ? '#cbd5e1' : '#475569' }}>
                    <strong>짧은 답:</strong> 네, 지원합니다. 초기 1회 로딩 후에는 네트워크 연결이 해제된 상태에서도 완벽하게 번호를 생성할 수 있습니다.
                  </p>
                  <p style={{ fontSize: '0.95rem', opacity: 0.85, lineHeight: '1.6', color: isDark ? '#cbd5e1' : '#475569' }}>
                    <strong>설명:</strong> Lotto Dreamer는 모던 프론트엔드 프레임워크인 React로 구축되어 백엔드 서버와의 실시간 데이터 통신을 요구하지 않는 클라이언트 사이드 렌더링(CSR) 방식을 채택하고 있습니다. 비행기 모드, 혹은 완전히 인터넷이 끊긴 환경에서도 브라우저의 연산 자원만을 독자적으로 활용하므로, 어떠한 외부 전송이나 응답 지연 없이 쾌적한 이용이 보장됩니다. 나아가 서버 전송이 일절 발생하지 않아 사용자의 개인정보와 생성 기록이 완벽하게 보호됩니다.
                  </p>
                </div>
              </div>
            </details>
            ...
            <details style={{ marginBottom: '1rem', background: isDark ? 'rgba(0,0,0,0.2)' : 'rgba(255,255,255,0.8)', padding: '18px', borderRadius: '12px', border: '1px solid ' + (isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)') }} itemscope itemprop="mainEntity" itemtype="https://schema.org/Question">
              ...
              <div itemscope itemprop="acceptedAnswer" itemtype="https://schema.org/Answer">
                <div itemprop="text">
                  <p style={{ marginTop: '12px', fontSize: '0.95rem', opacity: 0.85, lineHeight: '1.6', color: isDark ? '#cbd5e1' : '#475569' }}>
                    <strong>짧은 답:</strong> 60fps 파티클 애니메이션과 글래스모피즘(Glassmorphism) 시각 효과를 적용하여 프리미엄 사용자 경험(UX)을 제공합니다.
                  </p>
                </div>
              </div>
            </details>

            <details style={{ marginBottom: '2.5rem', background: isDark ? 'rgba(0,0,0,0.2)' : 'rgba(255,255,255,0.8)', padding: '18px', borderRadius: '12px', border: '1px solid ' + (isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)') }} itemscope itemprop="mainEntity" itemtype="https://schema.org/Question">
              <summary itemprop="name" style={{ fontWeight: 600, cursor: 'pointer', fontSize: '1.05rem', color: isDark ? '#e2e8f0' : '#334155' }}>
              ...
              <div itemscope itemprop="acceptedAnswer" itemtype="https://schema.org/Answer">
                <div itemprop="text">
                  <p style={{ marginTop: '12px', fontSize: '0.95rem', opacity: 0.85, lineHeight: '1.6', color: isDark ? '#cbd5e1' : '#475569' }}>
                    <strong>짧은 답:</strong> 아니오. 이 유틸리티는 반복 회수 제한 없이 100% 무료로 영구 제공됩니다.
                  </p>
                </div>
              </div>
            </details>
          ...
          <section aria-labelledby="disclaimer">
            <h2 id="disclaimer" style={{ fontSize: '1.4rem', marginBottom: '1.2rem', color: isDark ? '#f8fafc' : '#1e293b' }}>면책 조항 (Legal Disclaimer)</h2>
